import { InvalidThrottleIdentifierException, ThrottlingException } from '@domain/exceptions';
import { Injectable } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { ThrottleLimit } from './throttle-limit.vo';
import { IThrottlerService } from './throttler.service.interface';
    ...
    async trackRequest(
        ...
        if (record.count >= throttleLimit.getLimit) {
            throw new ThrottlingException(
                `Demasiadas solicitudes, por favor intenta nuevamente en ${Math.ceil((record.ttl - now) / 1000)} segundos`,
            );
        }
